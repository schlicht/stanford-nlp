...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            ...
            for word in sent:
                word_ids = []
                word_ids.append(self.start_of_word)
                for char in word:
                    word_ids.append(self.char2id[char])
                word_ids.append(self.end_of_word)
                sent_word_ids.append(word_ids)
            all_word_ids.append(sent_word_ids)
        return all_word_ids
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        wordIndicesPadded = pad_sents_char(wordIndices)
